(** {1 Bechamel, a simple and agnostic micro-benchmarking framework.}
    ...
    the developer prove and compare metrics for a given {b small} function. It's
    measuring the performance of something "small", like a system call. Bechamel
    does not do, as we say, a macro-benchmark which can show a performance
    regression or I/O congestion for instance.

    It just permits to assert that a simple call of a small function [fn1] can
    be faster than a call of another small function [fn2] (if you use a {i time}
    metric). In this way, it asserts that [fn1] should be more efficient than
    [fn2] and it lets the developer {b deduce} the best choice according to the
    runtime context.
    ...
    - An {i analyzer} which will analyze {i raw} metrics and give you a stated
      result
    ...
    From these small functions, we are able to make a test for each function and
    group them into one test:
    {[
    let test0 = Test.make ~name:"recursive" (Staged.stage @@ fun () -> fact0 120)

    let test1 =
      Test.make ~name:"imperative" (Staged.stage @@ fun () -> fact1 120)

    let test = Test.make_grouped ~name:"factorial" ~fmt:"%s %s" [ test0; test1 ]
    ]}

    The user is able to make multiple kinds of tests:
    - A simple one as we did above
    - An indexed one which can take an [int] as an argument. For instance, we
      can execute our [fact] function with a set of [int]s.
    ...
    distribution. More abstractly, Bechamel is able to record any metrics as far
    as the user is able to provide a {!modtype:Bechamel.S.MEASURE}.

    For instance, we will try to record the monotonic clock: it represents the
    absolute elapsed {i wall-clock} time since an arbitrary, fixed point in the
    past (usually, the time since the program began running).
    ...
    one time [fact0] and record the monotonic clock, you will probably get a
    {i partial} result which fluctuated a lot per run:
    {[
      $ cat >main.ml <<EOF
      > let rec fact0 x =
      >   if x = 0 then 1
    ...
    This is why Bechamel exists. From metrics, it can estimate the time spent by
    our test. There are 2 methods to do that:
    ...
    The main question behind this function is: I would like to compare what with
    what? By default, the benchmark iterates a {i certain} time on your
    ...
    code, [x = Measure.run] and [y = Instance.monotonic_clock]. OLS and RANSAC
    are algorithms which try to fit this curve. Then, [a] will become the time
    spent by our function for [x = 1] and this is what we want:
    ...
    Bechamel has many ways to show results, but the core still is agnostic to
    the system and does not need anything (like {!module:Unix}) to show results.
    However, the distribution comes with many possibilities:
    ...
    JavaScript support has the ability to show you more information (such as the
    curve for instance):
    {[
    let () =
