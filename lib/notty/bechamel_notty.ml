...
type 'a results = (Label.t, 'a result) Hashtbl.t
...
module Unit = struct
  ...
  let add instance unit =
    if String.length unit > 5 then Fmt.invalid_arg "A unit shoud be smaller than 5 bytes: %s" unit ;
    Hashtbl.add units (Measure.label instance) unit

  let label label = I.string A.empty (Label.to_string label)

  let unit_of_label label =
    try Hashtbl.find units label
    with Not_found -> Label.to_string label
end
...
module Order = struct
  ...
  let add instance order =
    Hashtbl.add orders (Measure.label instance) order

  let order_of_label label =
    try Hashtbl.find orders label
    with Not_found -> Increasing

  let compare_of_order order a b = match order with
...
let ols_value : predictor:Label.t -> Analyze.OLS.t -> image = fun ~predictor v ->
  ...
  | Some values ->
    match List.fold_left2
            (fun a v p -> if Label.equal p predictor then Some v else a)
            None
            values (Analyze.OLS.predictors v) with
    | Some value ->
      let s = Fmt.strf fmt_value value unit_responder unit_predictor in
      I.string attrs s
    | None -> assert false (* XXX(dinosaure): should never occur. *)

let ransac_value : Analyze.RANSAC.t -> image = fun v ->
  let attrs =
  ...
  let unit_responder = Unit.unit_of_label responder in
  let unit_predictor = Unit.unit_of_label predictor in
  let s = Fmt.strf fmt_value (Analyze.RANSAC.mean v) unit_responder unit_predictor in
  I.string attrs s
...
let hashtbl_choose hashtbl =
  ...
  try Hashtbl.iter (fun k' v' -> k := Some k' ; v := Some v' ; raise Break) hashtbl
    ; assert false
  with Break -> match !k, !v with
    | Some k, Some v -> k, v
    | _, _ -> assert false
...
module One = struct
  let image_of_header ~rect result =
    ...
    let responder = Analyze.OLS.responder v in
    let responder = Label.to_string responder in
    let max_length_of_names = Hashtbl.fold (fun name _ -> max (String.length name)) result 0 in
    ...
    grid
      ...
      ; [ sideline 1 1
        ...
        ; I.void (rect.w - (max_length_of_names + 4 + 2 + 2 + 1 + (String.length responder))) 1
        ; sideline 1 1 ]
      ; [ break_l 1 1
        ; line (max_length_of_names + 4) 1
        ; cross 1 1
    ...
    let field =
      grid
        [ [ sideline 1 1
          ...
          ; sideline 1 1
          ; I.(hsnap ~align:`Right (rect.w - 2 - 1 - (max_length_of_names + 4)) value)
          ; sideline 1 1 ] ] in
    img <-> field
  ...
  let best_and_worst_case (result:Analyze.OLS.t result) ~sort ~predictor ~rect =
    ...
    let values =
      List.map
        (fun (name, v) -> match Analyze.OLS.estimates v with
          | Some values ->
            List.fold_left2
              (fun a v p -> if Label.equal p predictor then (name, Some v) else a)
              (name, None)
              values (Analyze.OLS.predictors v)
          | None -> (name, None))
        tests in
    ...
    grid
      ...
      ; [ sideline 1 1
        ...
        ; I.void (rect.w - (max_length_of_names + 4 + 2 + 2 + 1 + (String.length best))) 1
        ; sideline 1 1 ]
      ; [ sideline 1 1
        ...
        ; I.void (rect.w - (max_length_of_names + 4 + 2 + 2 + 1 + (String.length worst))) 1
        ; sideline 1 1 ]
      ; [ corner_bl 1 1
        ; line (max_length_of_names + 4) 1
        ; break_b 1 1
  ...
  let image_of_ols_result
    ...
    = fun ?(sort= String.compare) ~rect ~predictor result ->
      ...
      let header_and_body =
        List.fold_left (image_of_field ~max_length_of_names ~rect ~predictor) header tests in

      let open Notty.Infix in
      header_and_body <-> (best_and_worst_case result ~sort:(Order.compare_of_label predictor) ~rect ~predictor)
end
...
module Multiple = struct
  ...
  let image_of_header ~rect (results : 'a results) =
    ...
    let max_length_of_names = Hashtbl.fold (fun name _ -> max (String.length name)) result 0 in
    let max_length_of_instances = List.fold_right (fun (label : Label.t) -> max (String.length (label :> string))) instances max_length_of_values in
    let max_length_of_fields = max max_length_of_values max_length_of_instances in

    let tt = List.map (fun _ -> [ break_t 1 1; line (max_length_of_fields + 4) 1 ]) instances |> List.concat in
    let tt = corner_tl 1 1 :: line (max_length_of_names + 4) 1 :: tt in
    let tt = tt @ [ corner_tr 1 1 ] in

    let cc = List.map
        (fun (instance : Label.t) ->
           let rest = max_length_of_instances - (String.length (instance :> string)) + 2 in
           [ sideline 1 1; I.(string A.empty (instance :> string) |> I.hpad 2 0); I.void rest 0 ])
        instances |> List.concat in
    let cc = sideline 1 1 :: I.(string A.(st italic) "name") :: I.void max_length_of_names 1 :: cc in
    let cc = cc @ [ sideline 1 1 ] in

    let bb = List.map (fun _ -> [ cross 1 1; line (max_length_of_fields + 4) 1 ]) instances |> List.concat in
  ...
  let image_of_ols_fields ~max_length_of_names ~max_length_of_instances ~rect ~predictor img (name, vs) =
    let values = List.map (ols_value ~predictor) vs in
    let max_length_of_fields = max max_length_of_values max_length_of_instances in

    let ll = [ sideline 1 1; I.(string A.empty name |> hpad 2 0); I.void (max_length_of_names + 4 - 2 - String.length name) 1 ] in
    let cc =
      List.map
        (fun value ->
           [ sideline 1 1; I.(hsnap ~align:`Right (max_length_of_fields + 4)) value; ])
        values
      |> List.concat in
  ...
  let image_of_ols_results
    ...
    = fun ?(sort= String.compare) ~rect ~predictor results ->
      ...
      let header_and_body =
        List.fold_left (fun img name ->
            let vs = Hashtbl.fold (fun label result a -> Hashtbl.find result name :: a) results [] in
            image_of_ols_fields ~max_length_of_names ~max_length_of_instances ~rect ~predictor img (name, vs))
          header tests in

      let max_length_of_fields = max max_length_of_values max_length_of_instances in

      let bottom = List.map (fun _ -> [ break_b 1 1; line (max_length_of_fields + 4) 1 ]) instances |> List.concat in
