...
let option_equal x = function Some x' -> x = x' | None -> false
...
let int_to_kind x =
  ...
  else if option_equal x process_cpu_time then `Process_cpu_time
  else if option_equal x thread_cpu_time then `Thread_cpu_time
