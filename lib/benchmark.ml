...
let cfg ?(run= 3000) ?(quota= Time.second 1.) ?(sampling= `Geometric 1.01) ?(stabilize= true) ?(start= 1) () =
...
stabilize_garbage_collector () ;
  ...
  while (not (exceeded_allowed_time cfg.quota init_time)) && !idx < cfg.run do
    ...
    for i = 0 to length - 1 do m0.(i) <- records.(i) () done ;

    runnable fn current_run ;

    for i = 0 to length - 1 do m1.(i) <- records.(i) () done ;

    m.(current_idx * (length + 1)) <- float_of_int current_run ;
    for i = 1 to length do m.((current_idx * (length + 1)) + i) <- m1.(i - 1) -. m0.(i - 1) done ;

    let next = match cfg.sampling with
      | `Linear k -> current_run + k
      | `Geometric scale ->
        let next_geometric = int_of_float (float_of_int current_run *. scale) in
        if next_geometric >= current_run + 1 then next_geometric else current_run + 1 in
  ...
  let stats =
    ...
    ; samples
    ; time= Mtime.span init_time final_time } in
