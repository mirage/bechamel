...
let of_ols_result ~x_label ~y_label ols =
  if not (Label.equal (Analyze.OLS.responder ols) x_label)
  then Fmt.invalid_arg "x-label:%a (expect:%a) does not exist in OLS result" Label.pp x_label Label.pp (Analyze.OLS.responder ols) ;
  if not (List.exists (Label.equal y_label) (Analyze.OLS.predictors ols))
  then Fmt.invalid_arg "y-label:%a does not exist in OLS result" Label.pp y_label ;
  ...
  | Some estimates ->
    ...
    let estimate =
      ...
      try List.iter2
            (fun predictor e ->
               if Label.equal y_label predictor
